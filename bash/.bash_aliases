...
alias ll='command exa -labg --git --group-directories-first --color=always'
alias lld='command exa -labgD --git --color=always'
alias ip='ip -c'
...
# Gentoo Specifics
alias ggit='git --git-dir=/gentoo-config --work-tree=/'
alias ggita='git --git-dir=/gentoo-config --work-tree=/ add'
alias ggits='git --git-dir=/gentoo-config --work-tree=/ status'
alias ggitd='git --git-dir=/gentoo-config --work-tree=/ diff'
alias ggitc='read -p "Enter commit message: " && git --git-dir=/gentoo-config --work-tree=/ commit -m "${REPLY}"'
alias gentoocrossdevi686='crossdev --b "2.34-r2" --k "5.4-r1" --g "9.3.0-r2" --l "2.32-r2" -oO /var/db/repos/crossdev --target i686-pc-linux-gnu'
...
alias gentooelog='cd /var/log/portage/elog/ && ll'
alias gentooecleanp='sudo eclean-dist -p -d -t2w'
alias gentooeclean='sudo eclean-dist -d -t2w'
alias gentoolvmsnapshot='read -p "comment for snapshot? " && sudo lvcreate -v -s -L 10G -n "ss_gentoo_$(date +%d_%m_%Y)_${REPLY}" gnulinux/gentooroot'
# kernel compile
alias gentookc='time make $(portageq envvar MAKEOPTS)'
# kernel install
alias gentooki='command ls -lahF --color=auto /lib/modules && read -p "Enter the kernel version: " kernelversion && make modules_install && emerge -q x11-drivers/nvidia-drivers && make install && sleep 3 && dracut -f /boot/initramfs-"${kernelversion}".img "${kernelversion}" && rm -I /boot/*old'
alias eq='equery -h'
